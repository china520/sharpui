...
ImplementTypeofInfo(Menu, Selector)
...
suic::Size Menu::MeasureOverride(const suic::Size& size)
{
    ...
    availableSize.cx += GetBorderThickness().SumLR() + GetPadding().SumLR();
    availableSize.cy += GetBorderThickness().SumTB() + GetPadding().SumTB();
...
void Menu::OnTextInput(suic::KeyboardEventArg& e)
{
}

void Menu::OnKeyDown(suic::KeyboardEventArg& e)
{
}
