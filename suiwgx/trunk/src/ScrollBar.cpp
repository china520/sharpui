...
void ThumbButton::OnRender(suic::DrawingContext * drawing)
{
    ...
    suic::ImageBrushPtr bkgnd = GetInternalValue(_T("Glyph"));
...
ImplementTypeofInfo(ScrollBar, ui::RangeBase)
...
void ScrollBar::OnPropertyChanged(suic::PropertyChangedEventArg& e)
{
    ...
    else
    {
        ...
        if (e.GetName().Equals(_T("Width")) || e.GetName().Equals(_T("Height")))
        {
            InternalInit();
        }
    }
...
{
    ui::StackPanelPtr midPanle(new ui::StackPanel());

    suic::FrameworkElementPtr decreaseBtn(new ui::RepeatedButton());
    suic::FrameworkElementPtr increaseBtn(new ui::RepeatedButton());
    suic::FrameworkElementPtr decreasePage(new ui::RepeatedButton());
    suic::FrameworkElementPtr increasePage(new ui::RepeatedButton());
    ThumbButton* pThumb(new ThumbButton());

    midPanle->AddChild(decreasePage);
    midPanle->AddChild(pThumb);
    midPanle->AddChild(increasePage);

    _panel = new ui::StackLayout();
    _panel->AddChild(decreaseBtn);
    _panel->AddChild(midPanle);
    _panel->AddChild(increaseBtn);

    _panel->SetOrientation(GetOrientation());
    midPanle->SetOrientation(GetOrientation());

    midPanle->SetHorizontalAlignment(HoriAlignment::Stretch);
    midPanle->SetVerticalAlignment(VertAlignment::Stretch);
    ...
    if (!iPage || iPage->Setters()->SetterColl().size() == 0)
    {
        iPage = dPage;
    }
    ...
    if (GetOrientation() == CoreFlags::Horizontal)
    {
        ...
        // 附加事件
        //
        _decreasePage.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnPageLeft));
        _increasePage.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnPageRight));
        _thumb.AddHandler(MouseMoveEvent, new suic::MouseEventHandler(this, &ScrollBar::OnHorzTrack), true);

        _decreaseBtn.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnLineLeft));
        _increaseBtn.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnLineRight));
    }
    else
    {
        _decreasePage.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnPageUp));
        _increasePage.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnPageDown));
        _thumb.AddHandler(MouseMoveEvent, new suic::MouseEventHandler(this, &ScrollBar::OnVertTrack), true);

        _decreaseBtn.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnLineUp));
        _increaseBtn.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnLineDown));
    }

    _thumb.AddHandler(MouseLButtonDownEvent, new suic::MouseEventHandler(this, &ScrollBar::OnBeginTrack), true);
    _thumb.AddHandler(MouseLButtonUpEvent, new suic::MouseEventHandler(this, &ScrollBar::OnEndTrack), true);

    //-------------------------------------------------------------------------------
    decreasePage->SetStyle(dPage);
    increasePage->SetStyle(iPage);
    pThumb->SetStyle(thumb);

    decreaseBtn->SetStyle(dBtn);
    increaseBtn->SetStyle(iBtn);
    ...
    //    // 附加事件
    //    //
    //    decreasePage->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnPageLeft);
    //    increasePage->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnPageRight);
    //    pThumb->MouseMove += suic::MouseEventHandler(this, &ScrollBar::OnHorzTrack);

    //    decreaseBtn->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnLineLeft);
    //    increaseBtn->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnLineRight);
    //}
    //else
    //{
    //    decreasePage->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnPageUp);
    //    increasePage->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnPageDown);
    //    pThumb->MouseMove += suic::MouseEventHandler(this, &ScrollBar::OnVertTrack);

    //    decreaseBtn->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnLineUp);
    //    increaseBtn->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnLineDown);
    //}

    //pThumb->MouseLButtonDown += suic::MouseEventHandler(this, &ScrollBar::OnBeginTrack);
    //pThumb->MouseLButtonUp += suic::MouseEventHandler(this, &ScrollBar::OnEndTrack);
    //-------------------------------------------------------------------------------
...
suic::Size ScrollBar::ArrangeOverride(const suic::Size& size)
{
    ...
    if (!rcThumb.Empty())
    {
        if (GetOrientation() == CoreFlags::Horizontal)
        {
            ...
            if (_thumb.GetHeight() > 0)
            {
                finalRect.top += (finalRect.Height() - _thumb.GetHeight()) / 2;
                finalRect.bottom = finalRect.top + _thumb.GetHeight();
            }
        ...
        else
        {
            ...
            if (_thumb.GetWidth() > 0)
            {
                finalRect.left += (finalRect.Width() - _thumb.GetWidth()) / 2;
                finalRect.right = finalRect.left + _thumb.GetWidth();
            }
            AddVisualChild(&_thumb);
            _thumb.Arrange(finalRect);
