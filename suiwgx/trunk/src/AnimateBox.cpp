...
ImplementTypeofInfo(AnimateBox, suic::Control)
...
void AnimateBox::Start()
{
    if (FrameCount() > 0)
    {
        _timerid = suic::SystemHelper::suiSetTimer(this, 10, suic::TimerFunc());
    }
}
...
void AnimateBox::OnPropertyChanged(suic::PropertyChangedEventArg& e)
{
...
void AnimateBox::OnLoaded(suic::LoadedEventArg& e)
{
    ...
    suic::ObjectPtr obj(GetInternalValue(_T("AutoStart")));
...
suic::Size AnimateBox::MeasureOverride(const suic::Size& size)
{
    ...
    return (!img ? suic::Size() : suic::Size(img->Width(), img->Height()));
}
...
void AnimateBox::OnTimer(int id)
{
    ...
    suic::SystemHelper::suiKillTimer(this, _timerid);
    _timerid = suic::SystemHelper::suiSetTimer(this, CurrentElapse(), suic::TimerFunc());
